...
handle_call({run, Args}, _From,
        S = #state{limit=Limit, sup=Sup, refs=Refs}) when Limit > 0 ->
    {ok, Pid} = supervisor:start_child(Sup, Args),
    Ref = erlang:monitor(process, Pid),
    {reply, {ok, Pid}, S#state{limit=Limit-1, refs=gb_sets:add(Ref, Refs)}};

handle_call({run, _Args}, _From, S=#state{limit=Limit}) when Limit =< 0 ->
    {reply, noalloc, S};
...
handle_call({sync, Args}, From, S = #state{queue=Queue}) ->
...
handle_cast({async, Args},
        S = #state{limit=Limit, queue=Queue}) when Limit =< 0 ->
...
handle_down_worker(Ref, S = #state{limit=Limit, sup=Sup, refs=Refs}) ->
